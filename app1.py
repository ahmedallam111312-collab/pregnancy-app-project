"""
Professional Pregnancy AI Assistant (Graduation Project - **Intelligent+ v6 - Clinical Reasoning**)
Features:
- **Premium Front-End:** Dashboard summary, interactive Plotly charts, icons, refined CSS.
- **Enhanced AI Logic:** Includes detailed patient history, **AI "thinks aloud" (differential diagnosis)**, **assesses urgency**, **intelligently parses messy OCR**, **considers medications**, **analyzes trends**.
...
try:
    ...
    # **IMPORTANT**: Download "DejaVuSans.ttf" from Google Fonts and place it in the same folder as this script.
    ARABIC_FONT_PATH = "DejaVuSans.ttf"
    FPDF_EXISTS = True
except ImportError:
    st.warning("مكتبات إنشاء PDF (fpdf2, arabic-reshaper, python-bidi) غير مثبتة. لن تعمل ميزة تحميل PDF.")
# --------------------------------
...
except Exception: TESSERACT_AVAILABLE = False
# --------------------------------
...
defaults = {
    ...
    'last_uploaded_id': None, 'ai_extracted_labs': {}
}
...
UTI,"ألم أو hرقان أثناء التبول، تكرار التبول، إلحاح بولي، ألم فوق العانة، رائحة بول كريهة","Urine Analysis, Urine Culture","WBCs <5, Nitrite Negative","حرارة، قشعريرة، ألم بالجانب (الكلى)، غثيان/قيء (علامات Pyelonephritis)","مضاد حيوي مناسب للحمل (مثل Cephalexin) فوراً مع إكمال الجرعة كاملة، شرب سوائل بكثرة (8-10 أكواب ماء يومياً)، تفريغ المثانة بانتظام"
...
RISK_FACTORS_LIST = [
    ...
    "أمراض المناعة الذاتية (مثل الذئبة)", "الحمل المتعدد (توأم أو أكثر)"
]
GSHEET_LAB_HEADERS = ["systolic_bp", "diastolic_bp", "fasting_glucose", "ogtt_1h", "ogtt_2h", "hba1c", "hb", "platelets", "alt", "ast", "creatinine", "urine_protein", "urine_ketones"]
# Added 'record_id' as the first column, plus medications and urgency
...
def get_patient_history_df(worksheet, patient_id_input):
    ...
    try:
        ...
        if not all(col in df.columns for col in required_cols_lower): return pd.DataFrame()
        df = df.replace('', pd.NA)
        ...
        numeric_cols = ['age', 'gravida', 'para', 'abortion', 'gestational_week', 'height_cm', 'pre_pregnancy_weight_kg', 'current_weight_kg', 'weight_gain_kg', 'pre_pregnancy_bmi', 'systolic_bp', 'diastolic_bp', 'fasting_glucose', 'ogtt_1h', 'ogtt_2h', 'hba1c', 'hb', 'platelets', 'alt', 'ast', 'creatinine']
    ...
    except Exception as e: print(f"Error fetching history: {e}"); return pd.DataFrame()
...
def ai_generate_final_report(patient_info, labs, history_df, symptoms_text, ocr_text):
    ...
    1.  **التشخيص التفريقي:** بناءً على الأعراض + التاريخ + عوامل الخطر، ما هي الاحتمالات (مع نسب تقديرية)؟ اشرح سبب تفكيرك.
    2.  **تنظيف وتفسير OCR + الدمج:** ادمج التحاليل اليدوية مع القيم الموثوقة من النص "{ocr_text}". حاول تفسير النص حتى لو كان غير منظم لاستخلاص القيم الهامة (سكر، ضغط، بروتين بول، Hb). طابق الأسماء مع الأعمدة المستهدفة.
    3.  **الاستخلاص المنظم للتحاليل:** أنشئ قائمة JSON **فقط** بالقيم المستخلصة والمطابقة للأعمدة المستهدفة. كن دقيقاً.
    4.  **تحليل الحالة الشامل:** قم بتقييم الحالة (وزن، ضغط، سكر، تاريخ، عوامل خطر، أدوية، اتجاهات).
    5.  **تقييم الإلحاح (Urgency):** بناءً على كل البيانات، صنف الحالة (متابعة روتينية، استشارة قريبة، تقييم فوري). **اذكر هذا التصنيف بوضوح.**
    6.  **التقرير النهائي (نصي):** اكتب تقريراً نصياً **مفصلاً واحترافياً** للمريضة يشمل:
        * ترحيب شخصي باسمها.
        * **التشخيص النهائي** الأكثر ترجيحًا مع **شرح تفصيلي** يربطه بالأعراض والتحاليل والتاريخ وعوامل الخطر. اذكر التشخيصات الأخرى التي تم استبعادها ولماذا.
        * **مستوى الإلحاح** وتفسير الإجراء المطلوب (مثال: "تقييم فوري يعني ضرورة التوجه للطوارئ حالاً").
        * **إرشادات وتدخلات عملية يمكن للمريضة البدء بها الآن:** قدم نصائح (غذاء، رياضة، مراقبة منزلية) **مرتبطة مباشرة بالنتائج والحالة والأدوية وأسبوع الحمل**. اذكر أمثلة محددة جداً (مثل أنواع الأطعمة). إذا كان هناك هبوط سكر، اذكر قاعدة الـ 15.
        * ذكر **علامات الخطر** الواضحة (Risk_Signs) التي تستدعي التواصل الفوري مع الطبيب **بغض النظر عن مستوى الإلحاح الحالي**.
        * **تأكيد واضح ومباشر على ضرورة المتابعة مع الطبيب.** (مثال: "هذا التقرير للمعلومات فقط ولا يغني عن زيارة طبيبكِ لمناقشة الخطة العلاجية المناسبة لحالتك.")
    ...
    try:
        ...
        for line in lines:
            ...
            if line_stripped.upper().startswith("URGENCY:"):
                ...
                json_started = True
                json_part += line_stripped[7:] # Capture rest of line if any
            elif line_stripped.startswith("--- REPORT TEXT ---"):
                report_started = True
                json_started = False # Stop JSON parsing if report starts
            elif json_started and line_stripped.endswith("```"):
                json_part += line_stripped[:-3] # Capture before end fence
                json_started = False
            elif json_started:
                json_part += line + "\n" # Keep original line breaks for JSON parsing
            elif report_started:
                report_part += line + "\n"

        if json_part:
            try:
                # Be more lenient with potential extra text around JSON
        ...
        elif not report_part and not json_part and full_response_text: report_text = full_response_text # Fallback
...
def save_record_to_gsheet(worksheet, record: dict):
    ...
    try:
        ...
        for lab_header in GSHEET_LAB_HEADERS:
             ...
             numeric_sheet_cols = ['systolic_bp', 'diastolic_bp', 'fasting_glucose', 'ogtt_1h', 'ogtt_2h', 'hba1c', 'hb', 'platelets', 'alt', 'ast', 'creatinine']
             ...
             # Ensure final_val is not None before assigning
        ...
        df = pd.DataFrame([record_to_save], columns=GSHEET_ALL_HEADERS) # Ensure correct order
...
def safe_get(record, key, default):
    ...
    val = record.get(key)
    # Check specifically for pandas NA or Python None before returning default
...
def create_pdf_bytes(report_text, patient_info, labs):
    ...
    reshaped_info = arabic_reshaper.reshape(info_text)
    ...
    for line in report_text.split('\n'):
        ...
        pdf.multi_cell(0, 7, bidi_line, align='R') # Use multi_cell for wrapping
...
st.markdown("""
    <style>
        ...
        body, .stApp, input, textarea, button, select, label, div[data-baseweb="select"] > div, .stDataFrame *, .stTable * { font-family: 'Cairo', sans-serif !important; direction: rtl; } /* Apply font globally */
        .stApp { background: linear-gradient(135deg, #FFF0F5 0%, #E6E6FA 100%); }
        .main > div { background-color: rgba(255, 255, 255, 0.95); padding: 2rem 3rem; border-radius: 25px; box-shadow: 0 15px 50px rgba(138, 43, 226, 0.12); border: 1px solid rgba(255, 255, 255, 0.3); } /* Enhanced card */
        h1, h2, h3 { color: #8A2BE2; font-weight: 700; text-shadow: 1px 1px 2px rgba(0,0,0,0.05); }
        h1 { text-align: center; margin-bottom: 2.5rem; }
        h3 { border-bottom: 2px solid #D8BFD8; padding-bottom: 0.6rem; margin-top: 2rem; margin-bottom: 1rem; display: flex; align-items: center;} /* Improved subheader */
        h3::before { content: '⭐ '; margin-left: 10px; font-size: 1.1em; color: #DA70D6; } /* Default icon */
        /* Specific icons */
        ...
        .stButton>button { border-radius: 30px; border: none; color: white; background: linear-gradient(45deg, #DA70D6, #8A2BE2); padding: 15px 40px; font-size: 1.1em; font-weight: 700; box-shadow: 0 6px 20px rgba(138, 43, 226, 0.35); transition: all 0.3s ease; cursor: pointer; } /* Larger button */
        .stButton>button:hover { transform: translateY(-5px) scale(1.05); box-shadow: 0 10px 30px rgba(138, 43, 226, 0.45); }
        .stTextInput input, .stNumberInput input, .stTextArea textarea, .stSelectbox div[data-baseweb="select"] > div { border-radius: 12px; border: 1px solid #D1C4E9 !important; box-shadow: inset 0 2px 4px rgba(0,0,0,0.06); transition: all 0.2s ease-in-out; padding: 10px 12px;} /* Enhanced inputs */
        ...
        .stMetric { background-color: #EDE7F6; padding: 1rem 1.5rem; border-radius: 15px; border: 1px solid #D1C4E9; text-align: center; box-shadow: 0 4px 8px rgba(0,0,0,0.05);} /* Added shadow to metric */
        ...
        /* Container styling */
        .stContainer { border: 1px solid #E6E6FA; border-radius: 15px; padding: 1.5rem; margin-bottom: 1.5rem; background-color: rgba(255, 255, 255, 0.65);} /* More transparent containers */
        /* Alert box styling */
        .stAlert { border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border: none; } /* Removed border */
        .stAlert [data-testid="stMarkdownContainer"] p { font-weight: bold; } /* Make alert text bold */
    </style>
...
with st.sidebar:
    ...
    # st.title("🤰 مساعد الحمل الذكي") # Title is now part of the SVG
    st.markdown("---")
...
if page == "التقييم الشامل":
    ...
    if patient_id:
        ...
        if not st.session_state.patient_history_df.empty:
            ...
            # ... (Dashboard metrics remain the same) ...
            ...
            st.dataframe(st.session_state.patient_history_df.tail(5).iloc[:, 1:].astype(str).reset_index(drop=True))
    ...
    with st.form("assessment_form"):
        ...
        with st.container(border=True):
            ...
            # ... (Patient Info Inputs remain the same) ...
            col_info1, col_info2 = st.columns(2);
            with col_info1: patient_name_input = st.text_input("✨ **اسمكِ بالكامل**", value=patient_name if patient_name else ""); age = st.number_input("**العمر**", 15, 60, value= int(safe_get(last_record, 'age', 25)), format="%d")
            with col_info2: gravida = st.number_input("الحمل رقم (G)", 0, value=int(safe_get(last_record, 'gravida', 1))); para = st.number_input("الولادات السابقة (P)", 0, value=int(safe_get(last_record, 'para', 0))); abortion = st.number_input("الإجهاض السابق (A)", 0, value=int(safe_get(last_record, 'abortion', 0)))
        ...
        with st.container(border=True):
            ...
            # ... (Measurement Inputs remain the same) ...
            col_meas1, col_meas2, col_meas3, col_meas4 = st.columns(4);
            gestational_week = col_meas1.number_input("أسبوع الحمل", 1, 45, value=int(safe_get(last_record, 'gestational_week', 8)))
            height_cm = col_meas2.number_input("**الطول (سم)**", 100, 250, placeholder="165", value=int(safe_get(last_record, 'height_cm', 160)))
            pre_preg_weight = col_meas3.number_input("**الوزن قبل الحمل (كجم)**", 30.0, 250.0, placeholder="65.0", value=float(safe_get(last_record, 'pre_pregnancy_weight_kg', 60.0)), format="%.1f")
        ...
        with st.container(border=True):
             ...
             # ... (Lab inputs remain the same structure but grouped) ...
             st.markdown("**العلامات الحيوية:**")
             lab_cols1 = st.columns([1,1]); systolic_bp = lab_cols1[0].number_input("ضغط الدم الانقباضي", value=None, placeholder="120"); diastolic_bp = lab_cols1[1].number_input("ضغط الدم الانبساطي", value=None, placeholder="80")
        ...
        # --- Submit Button ---
        # Ensure submit button is defined within the form's scope
    ...
    # Process image if uploaded and OCR state is empty (or if re-uploading a different file)
    ...
    if submitted:
        ...
        if not all([patient_id, patient_name_input, age, height_cm, height_cm > 0, pre_preg_weight, current_weight, symptoms_text]):
            ...
            with st.status("👩‍⚕️ يقوم المساعد الذكي بتحليل حالتكِ...", expanded=True) as status:
                ...
                labs = { "systolic_bp": systolic_bp, "diastolic_bp": diastolic_bp, "fasting_glucose": fasting_glucose, "ogtt_1h": ogtt_1h, "ogtt_2h": ogtt_2h, "hba1c": hba1c, "hb": hb, "platelets": platelets, "alt": alt, "ast": ast, "creatinine": creatinine, "urine_protein": urine_protein, "urine_ketones": urine_ketones }
                ...
                # Ensure all lab values passed to save function are serializable
                full_record = {
    ...
    if st.session_state.analysis_complete:
        ...
        # Display Urgency using st.alert for better visibility
        ...
        # Display report within a container
        with st.container(border=True):
        ...
        if FPDF_EXISTS:
            if os.path.exists(ARABIC_FONT_PATH):
                try:
                    # Gather info for PDF
                    pdf_patient_name = st.session_state.get('form_data', {}).get('patient_name_input', patient_name_input) # Get name from state
                    pdf_labs = st.session_state.get('ai_extracted_labs', labs) # Get AI labs or manual labs
                    pdf_info = patient_info.copy()
                    pdf_info['id'] = patient_id # Ensure ID is in patient_info
        ...
        if st.button("🔄 بدء تقييم جديد"):
            # Clear state for a truly new assessment, including patient ID
            keys_to_clear = list(st.session_state.keys())
            keys_to_keep = ['page'] # Keep only the page navigation state
...
# =========================== WEEKLY GUIDE PAGE ===========================
    ...
    default_week = 8
    hist_week = None
    # Use safe_get to handle potential missing keys or NA values in last_record
...
# =========================== FMC COUNTER PAGE ===========================
    ...
    # **FIX:** Ensure the full instruction text is displayed using markdown
    st.markdown("""
    **متى وكيف؟**
